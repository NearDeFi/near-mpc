...
pub fn p2p_tls_key() -> [u8; 32] {
    ...
    let public_key: attestation_contract_interface::types::Ed25519PublicKey =
...
pub fn mock_dto_dstack_attestation() -> attestation_contract_interface::types::Attestation {
    let quote = quote().into();
    let collateral_json_string = include_str!("../assets/collateral.json");
    let collateral = serde_json::from_str(collateral_json_string).unwrap();

    let tcb_info: attestation_contract_interface::types::TcbInfo =
        serde_json::from_str(TEST_TCB_INFO_STRING).unwrap();

    attestation_contract_interface::types::Attestation::Dstack(
        attestation_contract_interface::types::DstackAttestation::new(quote, collateral, tcb_info),
    )
}
